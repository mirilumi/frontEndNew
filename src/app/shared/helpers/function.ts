...
export class HelperFunctions {
    ...
    static createIdForNewEraStaff(lastUserId): any {
        if (lastUserId === undefined) {
            return '00000000-0000-0000-0002-000000000001';
        } else {
            let zerosToAdd;
            let incrementedPart = lastUserId.slice(24, 36);
            const afterincrementPart = ++ incrementedPart;
            if (afterincrementPart.toString().length < 12) {
                const lengthOfvar = afterincrementPart.toString().length;
                zerosToAdd = 12 - lengthOfvar;
            }
            const zeros = HelperFunctions.addZeros(0, zerosToAdd);

            return '00000000-0000-0000-0002-' + zeros + afterincrementPart;
        }
    }
    ...
    static DisplayToastInfoMessage(infoTitle: string, infoText: string): Promise<any> {
        return swal({
            type: 'info',
            title: infoTitle,
            text: infoText,
    ...
    static DisplayErrorMessageWithDetails(errorMessage: string, detailedMessage: string) {
        swal({
            ...
            html: '<p>' + errorMessage + '</p>' +
                  ...
                  '<div class="panel-collapse small" id="message">' + detailedMessage + '</div>',
            confirmButtonClass: 'btn btn-primary',
